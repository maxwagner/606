...
1.  In the first paragraph, several key findings are reported. Do these 
    ...
    sample) or *population parameters*?
        ...
        They appear to be simple statistics.
...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    assumption?
        ...
        I have to assume it truely is random and representative of the population. For example, the sample size must be large enough from each country proportianal to their population size. I think in the article is says the means are weighted according to population size.
...
3.  What does each row of Table 6 correspond to? What does each row of 
    `atheism` correspond to?
        ...
        Each row in table 6 is a country, and it's sample data. Each row in `Atheism` is a single reponse from one person (yes/no to Atheism).
...
4.  Using the command below, create a new dataframe called `us12` that contains
    ...
    us12_ <- table(us12$response)
    us12_[1] / (us12_[1] + us12_[2])
    ```
...
5.  Write out the conditions for inference to construct a 95% confidence
    ...
    The observations must be independent and it must pass the success-failure test. Both seem to be fulfilled.
...
7.  Using the `inference` function, calculate confidence intervals for the 
    ...
    ```{r}
    lb12 <- subset(atheism, nationality == "Lebanon" & year == "2012")
    ch12 <- subset(atheism, nationality == "China" & year == "2012")
    
    inference(lb12$response, est = "proportion", type = "ci", method = "theoretical", success = "atheist")
...
8.  Describe the relationship between `p` and `me`.
    ...
    `me` is at a maximum at `p` = 0.5, and decrease to 0 at `me` = 0 or 1.
...
9.  Describe the sampling distribution of sample proportions at $n = 1040$ and 
    ...
    *Hint:* Remember that R has functions such as `mean` to calculate summary
    ...
    The distribution has a center near 0.10, is normal with little to no skew.
...
10. Repeat the above simulation three more times but with modified sample
    ...
    `n` seems to increase the standard deviation while `p` seems to shift the center.
...
11. If you refer to Table 6, you'll find that Australia has a sample 
    ...
    p <- 0.1
    n <- 1040
    me_au <- 2 * sqrt(p * (1 - p) / n);me_au
    
    p <- 0.02
    n <- 400
    me_ec <- 2 * sqrt(p * (1 - p) / n);me_ec
    ```
    
    It is probably inaccurate to say that the margin or error is in the +-3-5% range given the wide range or `me` that can occur.
...
-   Answer the following two questions using the `inference` function. As 
    ...
    *Hint:* Create a new data set for respondents from Spain. Form
    ...
    Null: 2005 mean and 2012 mean are the same, Alt: 2005 mean and 2012 mean are not the same. The samples are less then 10% of the population, and it passes the success/failure test.
    
    ```{r}
    spain <- subset(atheism, nationality == "Spain")
    sp05 <- subset(spain, year == 2005)
    sp12 <- subset(spain, year == 2012)
    ...
    The CI's overlap from 2005 to 2012, so there is no statistical evidence that the atheism rate has changed.
    ...
    **b.** Is there convincing evidence that the United States has seen a
    ...
    The CI's do not overlap, so there is evidence that the atheism rate has changed.
...
-   If in fact there has been no change in the atheism index in the countries 
    ...
    *Hint:* Look in the textbook index under Type 1 error.
    ...
    95% of the time the CI would be correct, but 5% of the time a change would be expected.
