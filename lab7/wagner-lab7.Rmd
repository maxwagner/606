...
1.  What type of plot would you use to display the relationship between `runs` 
    ...
    The relationship definitely looks linear using only the above graph.
...
2.  Looking at your plot from the previous exercise, describe the relationship 
    ...
    It is a positive relationship, and is at least somewhat linear, but not overly strong. 
...
3.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
    ...
    I was able to get a few lines of: 145994.5, 148903.9, 146265.4, 166855.5, 140348.3. Each one using a slightly different angle.
...
4.  Fit a new model that uses `homeruns` to predict `runs`. Using the estimates 
    ...
    The slope above tells us that for every homerun, there is an increase of 1.83 runs. This means that if 1 person gets a homerun, they will score 1.83 for the team. This is definitely advantagous. 
...
5.  If a team manager saw the least squares regression line and not the actual 
    ...
    He would predict `r (.6305 * 5578) - 2789.2429` runs using the model. The Phillies had 5,579 at_bats and 713 runs, while the Tigers had 5568 at_bats and 787 runs. Most of the other teams near the Phillies were also below the regression line. My guess is that the model overestimates runs in this part of the data.
...
6.  Is there any apparent pattern in the residuals plot? What does this indicate
    ...
    There are about as many positive as negative residuals. This suggests a linear relationship.
...
7.  Based on the histogram and the normal probability plot, does the nearly 
    ...
    The histogram has a faint normal shape, but is possibly wrong for the conditions. The normal probability plot suggests the same, as it sways back and forth over the line.
...
-   Choose another traditional variable from `mlb11` that you think might be a 
    ...
    ```{r}
    qqplot(mlb11$bat_avg, mlb11$runs)
    ```
    
    There appears to be a strong linear relationship at first glance.
...
-   How does this relationship compare to the relationship between `runs` and 
    ...
    at_bats has a R^2 value of 0.3729, and bat_avg has a value of 0.6561 which. The greater R^2 value means that an additional 30% of the variability in runs is explained with batting averge in comparison to at_bat.
...
-   Now that you can summarize the linear relationship between two variables, 
    ...
    The next best after batting average seems to be hits, which makes sense as they are related to each other. The following looks at a graphical method, error checking, and finally a numerical summary.
...
-   Now examine the three newer variables. These are the statistics used by the 
    ...
    I have zero knowledge of baseball stats, but the correlation coefficents are extremely high compared to the highest of the other stats(bat_avg at .8). The best predicter seems to be new_obs. Using the same method as above with new_obs gives:
...
-   Check the model diagnostics for the regression model with the variable you 
    ...
    Using some of the information I gave above, we can see that it satisfies the conditions better than the old variables, thus seems to be a good predictor.
