...
1.  Make a histogram of men's heights and a histogram of women's heights.  How 
    ...
    Using descibe(), and the visuals of the graphs, it appears that the female heights are more closely packed than the males, with a range of only 35.7, versus 40.9, and a $\sigma$ of 6.54 versus 7.18. From the visuals alone, they are fairly similar.
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    Not all the points fall on the line, but they generally fit to the line. Compared to the real data, it is somewhat similar. The simulated data appears to deviate from the line more than the real data.
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    The simulated graphs provide evidence that the female heights data is nearly normal as they are visually similar.

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
    
    The weight data doesn't seem to be as perfect of a fit compared to the height data. But it looks to fall within the realm of possibilites. There are a few outliers. 
...
6.  Write out two probability questions that you would like to answer; one 
    ...
    How many females are below 155cm or above 175cm?
    
    In this case, the values for the simulated versus empircal are different. It is likely because of combining two sources of error.
    ```{r}
    (1 - pnorm(q = 175, mean = fhgtmean, sd = fhgtsd)) + (pnorm(q = 155, mean = fhgtmean, sd = fhgtsd))
    (sum(fdims$hgt > 175) / length(fdims$hgt)) + (sum(fdims$hgt < 155) / length(fdims$hgt))
    ```
    
    How many females have weights between 60kg and 80kg?
    
    The two methods give very different answers, meaning that the distribution is not exactly normal.
    
    ```{r}
    fwgtmean <- mean(fdims$wgt)
    fwgtsd <- sd(fdims$wgt)
    (pnorm(q = 80, mean = fwgtmean, sd = fwgtsd)) - (pnorm(q = 60, mean = fwgtmean, sd = fwgtsd))
    (sum(fdims$wgt < 80) / length(fdims$wgt)) - (sum(fdims$wgt < 60) / length(fdims$wgt))
    ```
    
    Between the height and weight distributions, it appears that the height is a more normal curve than weight.
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    Typically, a stepwise pattern indicates that the data is either rounded or discrete. Meaning it is likely categorical. I assume this is the case for elbow diameter. The age plot isn't as strongly patterned as the elbow plot, which I think makes sense being that the data is likely less rounded than the elbow data.
...
-   As you can see, normal probability plots can be used both to assess 
    ...
    A slope that increases from left to right indicates a right skew. The histogram confirms this.
