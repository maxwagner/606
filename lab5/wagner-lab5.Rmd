...
highlight about the relationship between these two variables?
    ...
    The plot depicts that the mean weight for smokers is slightly lower, but not definitely.
...
`length`.
    ...
    ggplot(nc) + geom_bar(aes(x = weight)) + facet_grid(habit ~ .)
    ```
    
    The sample sizes are large enough and the plots seem to be somewhat normal without much of a skew.

4.  Write the hypotheses for testing if the average weights of babies born to 
smoking and non-smoking mothers are different.

    Null: the avg weights are the same, Alt: the avg weights are not the same
...
mothers. Use a method of your choice, and explain how your method works.
    ```{r}
    library(psych)
    describeBy(nc[2:3], group = nc$mature)
    ```
    
    Used psych to get a group describe. Says the max age for younger mom's is 34, and the min age for mature moms is 35, so the cut off must be 34 to be a younger mom.
...
plain language.
    ...
    Does the weight gained changed if the mom is married or not?
    
    ```{r}
    inference(y = nc$gained, x = nc$marital, est = "mean", type = "ht", null = 0, alternative = "twosided", method = "theoretical")
    ```
    
    The p value is much higher than .05 so I do not reject the null, and accept that martial status is not significant to weight change.
