...
It reveals that more males have smoked 100 times than females.
...
The former plot shows that as the gen health condition gets worse, bmi gets higher. The following plot of bmi based on gender shows that males tend to have a higher bmi than females.
```{r e5}
...
-   Make a scatterplot of weight versus desired weight. Describe the 
    ...
    As the actual weight increases, the desired weight increases.
    ```{r}
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    If 'wdiff' is 0, it means the weight and desired weight are the same. If it is positive, it means they want to gain weight, if it is negative, it means they want to lose weight.
    ```{r}
    typeof(wdiff)
    ```
...
-   Describe the distribution of `wdiff` in terms of its center, shape, and 
    ...
    With the median at -10, and the mean at -14.59, and the graph depicting a right skew, it can be assumed that the vast majority of numbers fall below 0. A negative trend tells that people commonly want to lose weight, rather than gain it.
    ```{r}
    summary(wdiff)
    hist(wdiff, breaks = 50, col = "blue", xlim = c(-200, 100))
    ```
    
-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.
    
    DescribeBy from psych provides a bit too much information for this case, but looking at the widff mean and sd of each gender can tell quickly that females want to lose more weight than males do. The male mean / sd being -10.71 / 23.49, and the female being -18.15 / 24.00. This is compounded by the overall lower weight of females. A proportion is more appropiate in this case. The percentage of desired weight loss being *`r (-10.71 / 189.32) * 100`* for males, and *`r (-18.15 / 151.67) * 100`* for females.
    ```{r}
    library(psych)
    cdc_wdiff <- cbind(cdc, wdiff)
    wdiff_gender <- cdc_wdiff[,c(6:7, 9:10)]
    describeBy(wdiff_gender, group = wdiff_gender$gender)
...
-   Now it's time to get creative. Find the mean and standard deviation of 
    ...
    The mean and sd of all weights are 169.68 / 40.08, which makes the range `r 169.68 - 40.08` to `r 169.68 + 40.08`. 14152 cases fall in this range, which is roughly 70.76% of all cases.
    ```{r}
    total_cases <- nrow(cdc)
    weight_mean <- mean(cdc$weight)
    weight_sd <- sd(cdc$weight)
    new_cases <- nrow(subset(cdc, weight >= (weight_mean - weight_sd) & weight <= (weight_mean + weight_sd)))
    new_cases
