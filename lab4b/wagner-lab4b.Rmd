...
"typical" to mean.
    ...
    With a sample that is skewed or has outliers, the median is a more accurate middle than the mean. In the case of a single 60 element sample, the median is more likely to give a better center. The median in this case is `r median(samp)`.
...
Would you expect it to be similar? Why or why not?
    ...
    The chances of another student's distribution being identical to mine are nearly zero. However, the median should be fairly similar, but not necessarily exactly the same.
...
met for this to be true?
    ...
    It must be a simple random sample where the individual samples are independent. Like above, also normally distributed with a sample size large enough for the population.

## Confidence levels

4.  What does "95% confidence" mean? If you're not sure, see Section 4.2.2.

    What a 95% confidence interval means is that within the given range of values, there is a 95% chance it contains the population mean. The larger the sample, the smaller the interval, and vice-versa.
...
interval capture this value?
    ...
    The interval does contain the population mean.
...
population mean.
    ...
    Depending on the amount of people in the class, it should eventually equal out to 95% of the classes' intervals containing the population mean.
...
-   Using the following function (which was downloaded with the data set), 
    ...
    In this case I am explaining, there were 3 cases where the confidence level did not contain the population mean. This number could change on each load. 3/50 = `r 1- (3/50)`. It's unlikey that the proportion will be exactly the same each time. It is only a probability, not a definite.
...
-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?
        ...
        At 99% CI, the critical value is `r qnorm(1 - (1 - .99) / 2)`.
...
-   Calculate 50 confidence intervals at the confidence level you chose in the 
    ...
    In the few I looked at, all the sample contained the population mean. This is extremely likely, as the percentage is 99%. The value I found is higher than the 99% chances, but is very close.
    
    ```{r}
    lower_vector <- samp_mean - qnorm(1 - (1 - .99) / 2) * samp_sd / sqrt(n) 
    upper_vector <- samp_mean + qnorm(1 - (1 - .99) / 2) * samp_sd / sqrt(n)
    plot_ci(lower_vector, upper_vector, mean(population))
    ```
