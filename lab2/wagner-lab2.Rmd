...
1.  What does a streak length of 1 mean, i.e. how many hits and misses are in a 
    ...
    A streak of 1 would be either two H's, or two M's. A streak of 0 would be either 1 H, or 1 M.
...
2.  Describe the distribution of Kobe's streak lengths from the 2009 NBA finals. 
    ...
    It is right skewed, with his typical streak being 0, and his longest being 4.
...
-   If you were to run the simulation of the independent shooter a second time, 
    ...
    Without setting a seed, as I did above, you could expect the distribution to remain right skewed, but the max, and frequency of each streak will likely change each time. With 133 shots, the general shape will remain very similar, but with the exact numbers changing each time.
...
-   How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    The Kobe mean is `r kobe_streak <- calc_streak(kobe$basket); mean(kobe_streak)`, versus `r mean(streaks)` for the model. The median is `r median(kobe_streak)` for Kobe, and `r median(streaks)` for the model. The max is `r max(kobe_streak)` for Kobe, and `r max(streaks)` for the model. With a seed of 1, they model seems to fit fairly well, with similar means, medians, and max's. With some other seeds, the comparison may change slightly, but in general they seem similar. Certainly not conclusive proof, but interesting nonetheless. With more samples, the stats could look differntly.
