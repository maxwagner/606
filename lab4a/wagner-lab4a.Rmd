...
1.  Describe this population distribution.
    ...
    There are outliers in the data, which means the median will be a better indicator than the mean. It is also right skewed. The boxplot shows a large amount of outliers on the upper end.
...
2.  Describe the distribution of this sample. How does it compare to the 
    ...
    The distribution looks very similar to the population as a whole. However, with only 50 samples, the distribution could look quite a bit differently. I could set a seed, but I think it's useful to see the variation.
...
3.  Take a second sample, also of size 50, and call it `samp2`.  How does the 
    ...
    Like I said above, a sample of 50 will vary sample to sample. The mean of sample 1 is `r mean(samp1)`, while sample 2's mean is `r mean(samp2)`. These numbers could vary greatly depending on the sample. A larger sample size, of 1000, would provide a more accurate estimate.
...
4.  How many elements are there in `sample_means50`?  Describe the sampling 
    ...
    There are `r length(sample_means50)` elements in the distribution with an average mean of `r mean(sample_means50)`. I would assume a sample of 50,000 would not change the average mean too much. But looking below a 50,000 sample can be seen:
    ...
    mean(sample_means50_2)
    ```
    
    The average might be slightly more accurate, but will always center right around 1500.
...
5.  To make sure you understand what you've done in this loop, try running a 
    ...
    There will be 5000 elements, where the 1st 100 will be means, and the remaining will be set to NA.
...
6.  When the sample size is larger, what happens to the center?  What about the spread?
    ...
    The center of the samples are all relatively the same, but with larger sample sizes, the spread decreases. 
...
-   Take a random sample of size 50 from `price`. Using this sample, what is 
    ...
    The sample will likely be skewed and have outliers. So the median is probably the best way to find a middle. But a mean can be taken too.
...
-   Since you have access to the population, simulate the sampling 
    ...
    The histogram of prices looks like a normal distribution with no skew. The sample mean tends to be near `r options(scipen=999); round(mean(sample_means50),2)`, with the population mean being `r round(mean(price),2)`. 
...
-   Change your sample size from 50 to 150, then compute the sampling 
    ...
    The histogram of prices looks like a normal distribution with no skew, very similar to the 50 sample distribution. The sample mean tends to be near `r round(mean(sample_means150),2)`, which should be closer to the population mean of `r round(mean(price),2)` more often than the 50 sample distribution.
...
-   Of the sampling distributions from 2 and 3, which has a smaller spread? If
    ...
    The 50 sample IQR is `r IQR(sample_means50)`, and the 150 sample IQR is `r IQR(sample_means150)`. The 150 sample IQR is `r IQR(sample_means150) / IQR(sample_means50)` times smaller than the 50 sample IQR. To get estimates that are closer to the true value, a small spread would be preferred.
